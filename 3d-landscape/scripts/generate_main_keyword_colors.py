...
# Collect only the main keywords (dictionary keys, not the variants)
all_keywords = set()
...
# From embedded_keywords (use only the main keys)
all_keywords.update(kp.get('embedded_keywords', {}).keys())

# From speculative_keywords (use only the main keys)
all_keywords.update(kp.get('speculative_keywords', {}).keys())

# From critique_keywords (use only the main keys)
all_keywords.update(kp.get('critique_keywords', {}).keys())

# Create RGB-only mapping for main keywords only
rgb_only = {}
...
print(f"✓ Saved RGB colors for {len(rgb_only)} main keywords to 3DUMAP/data/colorsnew.json")
print("\nMain keywords:")
